...
🔹**백그라운드 작업**
	...
	- **메인 스레드와 분리** : 메인 스레드는 주로 사용자 인터페이스, 입력처리, 게임 로직 등 사용자와 직접 관련된 작업을 담당하는 반면 백그라운드 작업은 **파일 읽기, 데이터 처리, 게임 로직** 등 시간이 오래 걸릴 수 있는 작업을 처리하는 등 메인 스레드가 멈추거나 느려지는 것을 방지한다.
	- **비동기 처리** : 백그라운드 작업은 비동기로 실행되며, 작업이 완료된 후 결과를 메인 스레드로 전달한다.
	- **리소스 효율성** : 백그라운드 작업은 컴퓨터의 여러 코어를 활용해 작업을 병렬적으로 처리하면 성능을 최적화한다.
🔹**async & await****
	`async`는 비동기 메서드를 정의할 때 사용한다.
	`Task` 또는 `Task<T>` 반환 타입을 사용하여 비동기 작업을 수행한다.
	유니티에서는 Task 를 사용하기 보다는 `IEnumerator`를 사용하기도 한다.
	`await`는 비동기 작업이 완료될 때까지 기다리는 역할을 한다.
```csharp
...
public class AsyncExample : MonoBehaviour
{
    ...
    {
        await Task.Delay(3000); // 3초 대기 (메인 스레드 블로킹 X)
        Debug.Log("비동기 작업 실행 중...");
    }
}
/*
작업 시작!
